    ...
    sprintf "transactions - GET/POST - X-Provenance - ok",
        fun (fhir: FhirClient) () ->
            ...
            let partialProvenance =
                Provenance(
                    Agent = L [ Provenance.AgentComponent(Who=ResourceReference("Organization/" + organisations.O1.Id)) ],
                    Recorded = N DateTimeOffset.Now,
                    Reason = L [Coding(Display="Integration Testing")]
                )

            let patientCount1 = fhir.Search("Patient").Total.Value

            use br =
                fhir.OnBeforeRequest.Subscribe(
                   fun c -> c.RawRequest.Headers.Add("X-Provenance", partialProvenance.ToJson())
                )

            let mutable createdProvenanceLocation = ""
            use ar =
                fhir.OnAfterResponse.Subscribe(
                   fun c ->
                    createdProvenanceLocation <- c.RawResponse.Headers.Get("X-GoFHIR-Provenance-Location")
                )
            ...
            let checkTarget index (r: Resource) =
                ...
                let expected = sprintf "%s/%s/_history/%s" (r.ResourceType.ToString()) r.Id r.VersionId
            ...
            Expect.equal createdProvenance.Reason.[0].Display "Integration Testing" "reason"
